        ...
        if (DATUM != null) {
            ...
            if (datum.getPayment() != null)
                if (datum.getPayment().getTotal() != 0 ||
                    ...
                    if (datum.getPayment().getDiscount() > 0) {
                        Double totalValue = datum.getPayment().getFixed() + datum.getPayment().getTax() + datum.getPayment().getDistance() + datum.getPayment().getPeakAmount();
                        totalAmount.setText(NumberFormat.getNumberInstance(Locale.GERMAN).format(totalValue) + " " + Constants.Currency);
                    } else {
                        totalAmount.setText(NumberFormat.getNumberInstance(Locale.GERMAN).format(datum.getPayment().getTotal()) + " " + Constants.Currency);
                    }
                    payableAmount.setText(NumberFormat.getNumberInstance(Locale.GERMAN).format(datum.getPayment().getPayable()) + " " + Constants.Currency);
                }
